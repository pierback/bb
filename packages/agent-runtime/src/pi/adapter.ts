    ...
    ...createStandardAdapterMembers({
      ...
      buildProviderCommandPlan(command) {
        switch (command.type) {
          ...
          case "thread/fork": {
            // Pi's provider identity is its native session id. The bridge resolves
            // the source by that id, materializes the requested history at the
            // new BB thread's deterministic path, and reports the new native id.
            finishOpenProviderTurn({
              registry: turnState,
              threadId: command.threadId,
            });
