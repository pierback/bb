...
ALTER TABLE `environments` ADD `parent_environment_id` text;--> statement-breakpoint
ALTER TABLE `environments` ADD `parent_base_commit` text;--> statement-breakpoint
ALTER TABLE `environments` ADD `parent_had_uncommitted_changes` integer DEFAULT false NOT NULL;--> statement-breakpoint
...
CREATE TABLE `__new_environments` (
	...
	CONSTRAINT "environments_parent_shape_check" CHECK((
        ...
        (
          ...
          AND "__new_environments"."environment_provider_id" = 'git-worktree'
          AND "__new_environments"."provider_owns_path" = 1
        )
      ))
);
--> statement-breakpoint
INSERT INTO `__new_environments`("id", "name", "project_id", "host_id", "parent_environment_id", "parent_base_commit", "parent_had_uncommitted_changes", "path", "is_git_repo", "is_worktree", "branch_name", "base_branch", "default_branch", "merge_base_branch", "environment_provider_id", "environment_provider_plugin_id", "provider_owns_path", "environment_provider_selection", "environment_provider_instance_key", "retire_at", "teardown_attempt", "teardown_status", "teardown_message", "resource", "owner_thread_id", "attempt", "status_message", "pending_log", "claim_path", "status", "created_at", "updated_at") SELECT "id", "name", "project_id", "host_id", "parent_environment_id", "parent_base_commit", "parent_had_uncommitted_changes", "path", "is_git_repo", "is_worktree", "branch_name", "base_branch", "default_branch", "merge_base_branch", "environment_provider_id", "environment_provider_plugin_id", "provider_owns_path", "environment_provider_selection", "environment_provider_instance_key", "retire_at", "teardown_attempt", "teardown_status", "teardown_message", "resource", "owner_thread_id", "attempt", "status_message", "pending_log", "claim_path", "status", "created_at", "updated_at" FROM `environments`;--> statement-breakpoint
...
CREATE INDEX `environments_claim_idx` ON `environments` (`host_id`,`claim_path`);--> statement-breakpoint
...
ALTER TABLE `threads` ADD `source_seq_end` integer;--> statement-breakpoint
