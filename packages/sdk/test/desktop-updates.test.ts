...
import {
  ...
  resolvePierbackDesktopUserDataPath,
} from "../src/desktop-updates.js";

describe("Node desktop updates SDK", () => {
  it("reads and atomically changes the installed Pierback channel", async () => {
    const directory = await mkdtemp(resolve(tmpdir(), "pierback-sdk-updates-"));
  ...
  it("resolves Electron's Pierback user-data directory on macOS", () => {
    expect(
      resolvePierbackDesktopUserDataPath({
        homeDirectory: "/Users/tester",
        platform: "darwin",
      }),
    ).toBe("/Users/tester/Library/Application Support/Pierback");
  });
});
