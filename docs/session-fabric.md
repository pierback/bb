...
destination workspace evidence. Server and daemon protocol versions must match
exactly.
...
- The public handoff request accepts `source_worktree` and `isolated_worktree`.
  An isolated destination must already be provisioned on its selected host; the
  destination daemon inspects that workspace independently before the server
  records the cutover evidence.
