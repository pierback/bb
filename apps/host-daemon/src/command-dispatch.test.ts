...
describe("dispatchCommand", () => {
  ...
  it("preserves active work in the old environment during an idle release", async () => {
    const oldRuntime = createRuntime();
    const manager = new RuntimeManager({
      createRuntime: () => oldRuntime,
      provisionWorkspace: async () => createWorkspace("/tmp/bb-release-old"),
    });
    await manager.ensureEnvironment({
      environmentId: "env-old",
      workspacePath: "/tmp/bb-release-old",
    });
    oldRuntime.setActiveTurn("thread-1", "turn-old");
