...
import {
  ...
  type EnvironmentMigrationGitBranchTracking,
  type EnvironmentMigrationGitCheckout,
  type EnvironmentMigrationGitRemote,
...
import { ENVIRONMENT_MIGRATION_DIRECTORY } from "../environment-migration-storage.js";
import {
...
function migrationStagePath(args: {
  ...
  return path.join(
    args.dataDir,
    ENVIRONMENT_MIGRATION_DIRECTORY,
    args.side,
...
function sanitizePortableGitRemoteUrl(rawUrl: string): string | null {
  ...
  // Git's common SCP-style SSH syntax is not parsed by URL. It contains no
  // password field; the username is a transport identity and is portable.
  if (
    !value.includes("://") &&
    /^(?:[^@/:\s]+@)?[^/:\s]+:[^\s]+$/u.test(value)
  ) {
  ...
  if (parsed.protocol === "http:" || parsed.protocol === "https:") {
    // Never transfer embedded HTTP credentials or token-like query values.
    parsed.username = "";
    parsed.password = "";
  }
  parsed.search = "";
...
function uniquePortableGitRemoteUrls(stdout: string): string[] {
  return [
    ...new Set(
      stdout
        .split("\n")
        .map(sanitizePortableGitRemoteUrl)
        .filter((url): url is string => url !== null),
    ),
  ];
}

async function discoverGitRemoteState(args: {
  gitCheckout: EnvironmentMigrationGitCheckout;
  workspacePath: string;
}): Promise<{
  gitBranchTracking: EnvironmentMigrationGitBranchTracking | null;
  gitRemotes: EnvironmentMigrationGitRemote[];
}> {
  const remoteList = await runGit(["remote"], { cwd: args.workspacePath });
  const gitRemotes: EnvironmentMigrationGitRemote[] = [];
  for (const name of remoteList.stdout
    .split("\n")
    ...
    const [fetchResult, pushResult] = await Promise.all([
      runGit(["remote", "get-url", "--all", name], {
        cwd: args.workspacePath,
        allowFailure: true,
      }),
      runGit(["remote", "get-url", "--push", "--all", name], {
        cwd: args.workspacePath,
        allowFailure: true,
      }),
    ]);
    const fetchUrls = uniquePortableGitRemoteUrls(fetchResult.stdout);
    const pushUrls = uniquePortableGitRemoteUrls(pushResult.stdout);
    if (
      fetchUrls.length === 0 ||
      (pushResult.stdout && pushUrls.length === 0)
    ) {
      continue;
    }
    gitRemotes.push({ name, fetchUrls, pushUrls });
  }

  if (args.gitCheckout.kind === "detached") {
    return { gitBranchTracking: null, gitRemotes };
  }
  const [remoteResult, mergeResult] = await Promise.all([
    runGit(
      ["config", "--get", `branch.${args.gitCheckout.branchName}.remote`],
      { cwd: args.workspacePath, allowFailure: true },
    ),
    runGit(["config", "--get", `branch.${args.gitCheckout.branchName}.merge`], {
      cwd: args.workspacePath,
      allowFailure: true,
    }),
  ]);
  ...
  const gitBranchTracking =
    ...
    gitRemotes.some((remote) => remote.name === remoteName)
      ? { remoteName, mergeRef }
      : null;
  return { gitBranchTracking, gitRemotes };
}
...
async function sourceManifest(
  ...
  const gitCheckout = isGitRepo
    ...
    : null;
  const gitRemoteState =
    gitCheckout === null
      ? { gitBranchTracking: null, gitRemotes: [] }
      : await discoverGitRemoteState({ gitCheckout, workspacePath });
...
async function restoreGitRemoteState(args: {
  cwd: string;
  gitDir?: string;
  manifest: EnvironmentMigrationManifest;
}): Promise<void> {
  const prefix = args.gitDir ? ["--git-dir", args.gitDir] : [];
  await runGit([...prefix, "remote", "remove", "origin"], {
    cwd: args.cwd,
    allowFailure: true,
  });
  for (const remote of args.manifest.gitRemotes) {
    const [firstFetchUrl, ...additionalFetchUrls] = remote.fetchUrls;
    if (!firstFetchUrl) {
      throw new ExpectedCommandDispatchError(
        "migration_manifest_conflict",
        `Git remote ${remote.name} has no fetch URL`,
      );
    }
    await runGit([...prefix, "remote", "add", remote.name, firstFetchUrl], {
      cwd: args.cwd,
    });
    for (const fetchUrl of additionalFetchUrls) {
      await runGit(
        [...prefix, "remote", "set-url", "--add", remote.name, fetchUrl],
        { cwd: args.cwd },
      );
    }
    const [firstPushUrl, ...additionalPushUrls] = remote.pushUrls;
    if (firstPushUrl) {
      await runGit(
        [...prefix, "remote", "set-url", "--push", remote.name, firstPushUrl],
        { cwd: args.cwd },
      );
      for (const pushUrl of additionalPushUrls) {
        await runGit(
          [
            ...prefix,
            "remote",
  ...
  if (args.manifest.gitBranchTracking) {
    const checkout = args.manifest.gitCheckout;
    if (!checkout || checkout.kind === "detached") {
      throw new ExpectedCommandDispatchError(
        "migration_manifest_conflict",
        "Detached Git checkout cannot restore branch tracking",
      );
    }
    await runGit(
      [
        ...prefix,
        "config",
        `branch.${checkout.branchName}.remote`,
        args.manifest.gitBranchTracking.remoteName,
      ],
      { cwd: args.cwd },
    );
    await runGit(
      [
        ...prefix,
        "config",
        `branch.${checkout.branchName}.merge`,
        args.manifest.gitBranchTracking.mergeRef,
      ],
      { cwd: args.cwd },
    );
  }
...
async function initializeRestoredWorkspace(args: {
  ...
  await restoreGitRemoteState({ cwd: workingWorkspacePath, manifest });
  if (gitCheckout.kind === "branch") {
    await runGit(
