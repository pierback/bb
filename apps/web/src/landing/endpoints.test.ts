...
describe("marketing download redirect", () => {
  ...
  it("redirects macOS downloads to the current dmg asset", async () => {
    ...
    expect(response.headers.get("Location")).toBe(
      `${DOWNLOAD_RELEASE_ASSET_BASE_URL}/bb-mesh-0.0.26-arm64.dmg`,
    );
  });

  it("resolves macOS releases only through BB Mesh infrastructure", () => {
    expect(DESKTOP_DOWNLOADS.macos.versionFeedUrl).toBe(
      "https://updates.bb.staufingers.de/stable/desktop-version.json",
    );
    expect(DOWNLOAD_RELEASE_ASSET_BASE_URL).toBe(
      "https://updates.bb.staufingers.de/stable",
    );
    expect(DOWNLOAD_FALLBACK_URL).toBe(
