...
export function registerActionsCommands(
  ...
  parent
    .command("continue-after-rate-limit [id]")
    .description("Continue the current request after a provider rate limit")
    .option("--self", "Target the current thread (from BB_THREAD_ID)")
    .option(
      "--request-id <id>",
      "Require this failed client request id before continuing",
      ...
      action(
        async (id: string | undefined, opts: ThreadRetryCommandOptions) => {
          ...
          if (failedRequestId === undefined) {
            throw new Error(
              `Thread ${threadId} cannot continue after a provider rate limit (${status.reason}).`,
            );
          }
          ...
          if (outputJson(opts, output)) return;
          console.log(
            `Thread ${threadId} provider rate limit continuation requested manually`,
          );
        },
      ),
    );
