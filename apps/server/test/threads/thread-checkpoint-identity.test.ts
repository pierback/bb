...
describe("checkpoint identity through runtime, ingestion, and editing", () => {
  it.each(["fork", "resume"] as const)(
    "preserves the %s session and checkpoint through SQLite and branch creation",
    async (construction) => {
      await withTestHarness(async (harness) => {
        ...
        try {
          ...
          const branchStart = await waitForQueuedCommand(
            harness,
            (queued) =>
              queued.command.type === "thread.start" &&
              queued.command.threadId === fork.id,
          );
          if (branchStart.command.type !== "thread.start")
            throw new Error("Expected branch thread start");
          const branch = await runtime.startThread({
            ...args,
            threadId: fork.id,
            fork: branchStart.command.fork,
          });
          await reportQueuedCommandSuccess(harness, branchStart, branch);
          expect(
          ...
          expect(branchStart.command.fork).toEqual({
            sourceProviderCheckpointId: "turn-fx-1",
