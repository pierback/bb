...
if (cliArgs[0] === "machine" && cliArgs[1] === "enroll") {
  const bootstrapEnv = option("--bootstrap-env");
  const bundle = JSON.parse(process.env[bootstrapEnv]);
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ hostId: bundle.hostId, hostKey: "durable-test" }),
  );
  fs.writeFileSync(
    path.join(dataDir, "config.json"),
    JSON.stringify({ serverUrl: bundle.serverUrl }),
  );
  process.exit(0);
}
...
function writeServerInstallTools(
  ...
  options: { invocationPath?: string; statusServerUrl?: string } = {},
...
`,
  ...
  it.each([{ container: false }, { container: true }])(
    ...
    ({ container }) => {
      const fixture = createFixture();
      writeServerInstallTools(fixture, 200, {
        invocationPath: join(fixture.dataDir, "daemon-invocation"),
      });
      if (container) {
        writeExecutable(
          join(fixture.binDir, "uname"),
  ...
  it("rejects a missing coordinator artifact without registry fallback", () => {
    const fixture = createFixture();
