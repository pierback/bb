...
function handoffPrepareRequest(
  ...
  overrides: {
    ...
    destinationWorkspaceDisposition?: "isolated_worktree" | "source_worktree";
  ...
  return {
    ...
    destinationEnvironmentId:
      overrides.destinationEnvironmentId ?? fixture.environment.id,
    destinationHostId: overrides.destinationHostId ?? fixture.host.id,
    destinationModel: {
    ...
    destinationWorkspaceDisposition:
      overrides.destinationWorkspaceDisposition ?? "source_worktree",
    idempotencyKey:
      overrides.idempotencyKey ?? "public-session-handoff-request-0001",
  } as const;
}
...
describe("Session Fabric public routes", () => {
  ...
  it("seals an independently observed isolated destination workspace", async () => {
    await withTestHarness(async (harness) => {
      const fixture = seedThreadFixture(harness);
      const sourceBinding = seedFabricBinding(harness, fixture);
      const destinationEnvironment = seedEnvironment(harness.deps, {
        hostId: fixture.host.id,
      ...
      const sourceRootPath =
        fixture.environment.path ?? "/tmp/test-environment";
      const destinationRootPath = destinationEnvironment.path!;
      const destinationWorkspaceState = {
        ...hostWorkspaceState(fixture),
        rootPath: destinationRootPath,
        worktreeId: `worktree:${destinationEnvironment.id}`,
      };
      ...
      const responder = registerHostRpcResponder(harness, {
        ...
        handle: (rpcRequest) => {
          ...
          if (
            command.type === "session.handoff.inspect_destination_workspace"
          ) {
            expect(command).toEqual({
              type: "session.handoff.inspect_destination_workspace",
              environmentId: destinationEnvironment.id,
              workspaceContext: {
                workspacePath: destinationRootPath,
                workspaceProvisionType: "unmanaged",
              },
            });
            return { ok: true, result: destinationWorkspaceState };
          }
          throw new Error(`Unexpected command ${command.type}`);
        },
      });
        ...
        `/api/v1/session-fabric/bindings/${sourceBinding.id}/handoffs`,
        {
          ...
          body: JSON.stringify(
            handoffPrepareRequest(fixture, destinationThread.id, {
              ...
              idempotencyKey: "public-isolated-handoff-request-0001",
            }),
          ),
        },
      );
      ...
      const prepared =
        sessionFabricHandoffPrepareResponseSchema.parse(responseBody);
      expect(prepared.capsule.expectedWorkspaceState).toMatchObject({
        hostId: fixture.host.id,
        rootPath: destinationRootPath,
        worktreeId: destinationWorkspaceState.worktreeId,
      });
      expect(prepared.capsule.expectedWorkspaceState.worktreeId).not.toBe(
        `worktree:${fixture.environment.id}`,
      );
      expect(
        responder.requests.filter(
          (request) =>
