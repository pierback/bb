...
describe("public environments", () => {
  it("rejects moving either side of a live nested environment family", async () => {
    await withTestHarness(async (harness) => {
      const source = seedHostSession(harness.deps, {
        id: "host-nested-environment-source",
      });
      const target = seedHostSession(harness.deps, {
        id: "host-nested-environment-target",
      });
      const { project } = seedProjectWithSource(harness.deps, {
        hostId: source.host.id,
      });
      const parent = seedEnvironment(harness.deps, {
        hostId: source.host.id,
        projectId: project.id,
        path: "/source/parent",
        workspaceProvisionType: "managed-worktree",
      });
      const child = seedEnvironment(harness.deps, {
        hostId: source.host.id,
        managed: true,
        parentBaseCommit: "a".repeat(40),
        projectId: project.id,
        parentEnvironmentId: parent.id,
        path: "/source/parent/child",
        workspaceProvisionType: "managed-worktree",
      });

      for (const environmentId of [parent.id, child.id]) {
        ...
        await expect(readJson(response)).resolves.toMatchObject({
          code: "nested_environment_migration_unsupported",
        });
      }
    });
  });
