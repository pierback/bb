...
export function resolveRuntimeWorkspaceProvisionType(
  environmentProviderId: string | null,
): WorkspaceProvisionType {
  return (
    resolveDeprecatedWorkspaceProvisionType(environmentProviderId) ??
    "unmanaged"
  );
}
...
export function toEnvironmentResponse(row: EnvironmentRow): Environment {
  return {
    ...
    hostId: row.hostId,
    parentEnvironmentId: row.parentEnvironmentId,
    parentBaseCommit: row.parentBaseCommit,
    parentHadUncommittedChanges: row.parentHadUncommittedChanges,
    path: row.path,
