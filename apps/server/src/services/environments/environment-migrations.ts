...
export class EnvironmentMigrationCoordinator {
  ...
  async begin(
    ...
    if (
      ...
      hasNonDestroyedChildEnvironments(this.deps.db, environment.id)
    ) {
      throw new ApiError(
        409,
        "nested_environment_migration_unsupported",
        "Move the complete environment family together; individual nested environments cannot be moved between hosts",
      );
    }
