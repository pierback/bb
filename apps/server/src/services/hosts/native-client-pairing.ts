...
const DEFAULT_MAX_PENDING_REQUESTS_PER_SOURCE = 4;
...
export interface NativeClientPairingCreateInput {
  deviceName: string;
  sourceKey: string;
}
...
export class NativeClientPairingService {
  ...
  private readonly maxPendingRequestsPerSource: number;
  ...
  constructor(private readonly options: NativeClientPairingServiceOptions) {
    ...
    this.maxPendingRequestsPerSource =
      options.maxPendingRequestsPerSource ??
      DEFAULT_MAX_PENDING_REQUESTS_PER_SOURCE;
  ...
  create(input: NativeClientPairingCreateInput): NativeClientPairingRequest {
    this.removeExpired();
    const sourcePendingCount = [...this.pending.values()].reduce(
      (count, pairing) =>
        count + (pairing.sourceKey === input.sourceKey ? 1 : 0),
      0,
    );
    if (sourcePendingCount >= this.maxPendingRequestsPerSource) {
      throw new NativeClientPairingError("source_capacity_exceeded");
    }
