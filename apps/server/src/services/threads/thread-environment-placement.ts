...
async function nestedWorktreeProviderInputs(
  deps: PlacementDeps,
  environment: EnvironmentRow,
): Promise<JsonValue> {
  if (environment.path === null) {
    throw new Error("Validated parent environment is missing a workspace path");
  }
  ...
  recordEnvironmentCurrentBranch(deps.db, deps.hub, environment.id, {
    branchName: inspection.checkout.branchName,
    defaultBranch: inspection.defaultBranch ?? inspection.checkout.branchName,
  });
  return worktreeProviderInputs({
    kind: "named",
    name: inspection.checkout.branchName,
  });
}
  ...
  switch (resolvedEnvironment.type) {
    ...
    case "host": {
      ...
      if (workspace.type !== "unmanaged") {
        ...
        if ("baseBranch" in workspace) {
          inputs = worktreeProviderInputs(workspace.baseBranch);
        } else {
          const parentEnvironment = resolvedEnvironment.parentEnvironment;
          if (parentEnvironment === null) {
            throw new Error(
              "Validated nested workspace is missing its parent environment",
            );
          }
          inputs = await nestedWorktreeProviderInputs(deps, parentEnvironment);
        }
