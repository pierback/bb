  ...
  const { stdout } = await execFileAsync(
    ...
    [
      ...
      [
        ...
        "}",
        "",
        "if bb_mesh_wait_for_desktop_process_quiescence \\",
...
test("fails closed when an unmatched coordinator keeps the port healthy", async () => {
  const { stdout } = await execFileAsync("/bin/bash", [
    ...
    [
      ...
      'source "$1"',
      "state_index=0",
      "bb_mesh_desktop_processes_are_running() { return 1; }",
      "bb_mesh_desktop_runtime_is_recorded() { return 1; }",
      "bb_mesh_desktop_coordinator_is_healthy() { return 0; }",
      "bb_mesh_signal_desktop_processes() { return 0; }",
      "bb_mesh_stop_desktop_runtimes() { return 0; }",
      "sleep() { state_index=$((state_index + 1)); }",
      "if bb_mesh_wait_for_desktop_cutover_quiescence 4 2; then",
      '  result="success"',
      "else",
      '  result="failure"',
      "fi",
