...
## BB Mesh desktop update channel

The installed BB Mesh app follows either the `stable` or `canary` signed
release feed. Read or change this Mac's channel with:
...
`setChannel(channel)`. Both surfaces update the same local
`desktop-update-channel.json` preference observed by the running BB Mesh app.
